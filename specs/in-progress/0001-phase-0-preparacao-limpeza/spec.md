...
- [ ] T008 [OPS] [US-002] Criar e publicar `archived` a partir da main, verificando com .claude/scripts/phase0/assert-archived-frozen.test.js — Refs: US-002, FR-001, NFR-002, AC-001 — Depends: T001, T006
  - [ ] **PREP**: Confirmar que a main não tem alterações não commitadas e que `archived` ainda não existe no remoto. Interromper se qualquer condição falhar, conforme a seção 7.
  - [ ] **EXECUTE**: Criar `archived` apontando ao commit atual da main e publicá-la.
  - [ ] **VERIFY**: Executar `assert-archived-frozen.test.js` e observar GREEN. Esta é a barreira da fase: nenhuma remoção começa sem este resultado.
  - [ ] **EVIDENCE**: Registrar o SHA congelado como baseline lida por `assert-baseline-untouched.test.js`, a saída de GREEN e o resultado de `git ls-remote --heads origin archived` na seção 12.
