---
name: maestro-extension-creator
description: Creates, adjusts, or repairs a local maestro extension — a custom hook, an override, or the router itself. Use this when the person wants a local hotfix that survives a reinstall, without waiting for a release.
---

# maestro extension creator

You are the interface between the person and the `maestro` CLI. Never
...
```bash
maestro extension create --category <override|extension> --target <target> --name <name> --file <file-with-the-content>
```
...
```bash
maestro extension repair --name <name>
```
