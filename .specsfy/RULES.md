...
- src/**/*.ts, tests/**/*.ts, comentários, identificadores e artefatos empacotados pelo common-rules (skills locais, textos de CLI) são em inglês. Exceção técnica confirmada: specs/**/*.md permanecem em português, porque os scripts validadores do framework Specsfy (ferramenta externa, .agents/skills/specsfy-*/scripts/*.mjs) têm hardcoded títulos de seção e frases em português (ex.: validate_spec.mjs, verify_acceptance.mjs) — traduzir as specs quebraria a esteira de validação inteira. .specsfy/Spec.md também não é traduzido: é publicado pela própria ferramenta Specsfy, não por este projeto. A mesma exceção cobre o bloco gerenciado de docs/**/*.md (entre <!-- specsfy:documentator:start --> e --end -->): build_documentation.mjs (também externo, .agents/skills/specsfy-documentator/scripts/) embute prosa em português fixa nos títulos que gera (ex.: "Documentação técnica", "Arquitetura", "Testes") e seu modo --check falha ("Documentação desatualizada") se o conteúdo divergir do que o próprio gerador produz — mesma classe de dependência rígida do specs/*.md. Conteúdo humano em docs/ fora desse bloco gerenciado continua livre para inglês. Confirmado na SPEC-0012 (specs/in-progress/0012-regra-common-rules-idioma-padrao-e-config-yaml-sempre-presente/spec.md, DEC-005) via leitura direta do script.
